...
func (client *stateSyncerClient) acceptSyncSummary(proposedSummary message.SyncSummary) (block.StateSummaryMode, error) {
	...
	if !isResume {
		...
		if client.lastAcceptedHeight+client.stateSyncMinBlocks > proposedSummary.Height() {
			...
			if err := client.StateSyncClearOngoingSummary(); err != nil {
				return block.StateSummaryStopped, fmt.Errorf("failed to clear ongoing summary after skipping state sync: %w", err)
			}
	...
	if err := client.metadataDB.Put(stateSyncSummaryKey, proposedSummary.Bytes()); err != nil {
		return block.StateSummaryStopped, fmt.Errorf("failed to write state sync summary key to disk: %w", err)
	}
	if err := client.db.Commit(); err != nil {
		return block.StateSummaryStopped, fmt.Errorf("failed to commit db: %w", err)
	}
