...
func TestNetworkDoesNotConnectToItself(t *testing.T) {
	...
	assert.NoError(t, n.Connected(selfNodeID, &version.Application{Major: 1, Minor: 0, Patch: 0}))
...
func TestRequestMinVersion(t *testing.T) {
	...
	requestBytes, err := message.RequestToBytes(codecManager, requestMessage)
	assert.NoError(t, err)
	assert.NoError(t, net.Connected(nodeID, &version.Application{Major: 1, Minor: 7, Patch: 1}))

	// ensure version does not match
	responseBytes, _, err := client.RequestAny(&version.Application{Major: 2, Minor: 0, Patch: 0}, requestBytes)
	...
	responseBytes, _, err = client.RequestAny(&version.Application{Major: 1, Minor: 0, Patch: 0}, requestBytes)
	assert.NoError(t, err)
